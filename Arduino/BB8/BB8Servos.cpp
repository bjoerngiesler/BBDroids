...
Result BB8Servos::start(ConsoleStream *stream) {
  ...
  for(unsigned int i=0; i<numBps; i++) {
    ...
    if(stream) stream->print(String("scan successful at ") + bps + "bps, enumerating...");
  ...
  for(auto& s: servos_) {
    ...
    dxl_.writeControlTableItem(ControlTableItem::POSITION_P_GAIN, s.first, 3000);
  }
